...
VertexBuffer Rendering::To(sf::RenderWindow &window, Vector3 Camera_Pos, Vector3 Camera_Look, Vector3 Up, RenderBuffer &buffer, int View_Angle, int HEIGHT, int WIDTH, int View_Distance)
{
	...
	for (int i = 0; i < buffer.getSize(); i++)
	{
		...
		if (abs(Anglexz) < float(View_Angle) / 2) 
		{
			...
			{
				scene.setValue(i, Vector3(Anglexz / Angle_bitw + float(WIDTH) / 2, -Angleyz / Angle_bith + float(HEIGHT) / 2, TempPoint.GetValues(2)));
			}
		}
	}
	return scene;
...
VertexBuffer Rendering::To(sf::RenderWindow &window, Vector3 Camera_Pos, Vector3 Camera_Look, Vector3 Up, FaceBuffer &buffer, int View_Angle, int HEIGHT, int WIDTH, int View_Distance)
{
	float Angle_bith = (float)View_Angle / (float)HEIGHT;
	float Angle_bitw = (float)View_Angle / (float)WIDTH;
	float x_offset = 0;
	...
	float Anglexz = 0;
	Vector3 Lookat(0, 0, 1);
	Vector3 TempPoint(0, 1, 0);
	Vector3 a(0, 0, 0);
	Vector3 b(0, 0, 0);
	Triangle t;
	Vector3 norm(0, 0, 0);
	VertexBuffer scene(buffer.getTaken()*3);
	for (int i = 0; i < buffer.getTaken(); i++)
	{
		t = buffer.getValue(i);
		if (Lookat.angleBetween(t.getNormal()) >= 90) continue;
		for (int j = 0; j < 3; j++)
		{
			if (TempPoint.equals(t.getValue(j)))
			{
				scene.setValue(i*3+j, Vector3(Anglexz / Angle_bitw + float(WIDTH) / 2, -Angleyz / Angle_bith + float(HEIGHT) / 2, 0));
				continue;
			}
			TempPoint = t.getValue(j);
			a = Vector3(TempPoint.GetValues(0), 0, TempPoint.GetValues(2));
			b = Vector3(0, TempPoint.GetValues(1), TempPoint.GetValues(2));
			x_offset = ((Lookat*a) / (a.length() * Lookat.length()));
			Anglexz = acos(x_offset)*PIr;
			...
			if (abs(Anglexz) < float(View_Angle) / 2)
			{
				...
				if (abs(Angleyz) < float(View_Angle) / 2)
				{
					scene.setValue(i*3+j, Vector3(Anglexz / Angle_bitw + float(WIDTH) / 2, -Angleyz / Angle_bith + float(HEIGHT) / 2, TempPoint.GetValues(2)));
				}
			}
		}
	}
