...
int main()
{
	...
	FaceBuffer faceBuffer(buffer.getSize());
	faceBuffer.createCube(20, Vector3(100, 0, 200), 0);

	float sensitivity = 1.1;
	CameraHandle<FaceBuffer> cameraHandle(500,500,sensitivity,faceBuffer); // THE CAMERA WILL ROTATE INCORRECTLY AT THE FIRST FRAME !
	...
	while (window.isOpen()){
		...
		while (window.pollEvent(event))
		{
			...
			case sf::Event::KeyPressed:
				cameraHandle.KeyboardMovedCamera(event,step);
				break;

			case sf::Event::MouseMoved:
				cameraHandle.MouseMovedCamera(event);
				break;
		...
		if (renderMethod == "To")
		{
			VertexBuffer Screen_buffer = Rendering::To(window, Camera_Pos, Camera_Look, up, faceBuffer, View_Angle, Screen_Width, Screen_Height, View_Distance);
			Screen_buffer.DrawBufferTriangles(window);
		}
	}
}